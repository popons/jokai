...
- 2026-04-01 12:49:40 +0900 [調査] issue の `status` は `draft/published` を持ち `published_at` も一覧表示されるが、`web-src/main.js` に publish UI はなく、`src/main.rs` の create/save/duplicate も `draft` 前提のまま。現状「公開前の下書き」に対応する発行操作は未実装と判断。
- 2026-04-01 13:05:15 +0900 [着手] 発行フロー実装を開始。`build-error.txt` `test-error.txt` `clippy-error.txt` は成功、`build-error-win.txt` は未作成を確認し、API と read-only UI を追加する方針で着手。
- 2026-04-01 13:18:20 +0900 [実装] `src/main.rs` に `POST /api/issues/{id}/publish` と draft/published 定数、発行済み mutation ガードを追加。save/delete に加え item/block 添付追加削除も `published` で拒否するよう揃えた。
- 2026-04-01 13:18:20 +0900 [実装] `web-src/main.js` と `web-src/app.css` に発行ボタン、発行済み read-only editor、`複製して編集` 導線、一覧の `下書き/発行済み` 表示、競合時の再読込、disabled スタイルを追加した。`context/repo-map.md` と `AGENTS.md` も current truth に更新。
- 2026-04-01 13:20:34 +0900 [ビルド] `cargo fmt` と `npm run build` を実行。保存後 1 秒待機し、`build-error.txt` `test-error.txt` `clippy-error.txt` は成功、`build-error-win.txt` は未作成を確認した。
- 2026-04-01 13:20:34 +0900 [検証] `curl http://localhost:12040/api/issues` で既存 issue 一覧取得を確認し、Chrome DevTools で draft `/edit` に `発行する` が出ること、mock した `published` 応答では `/edit` が read-only 化され `複製して編集` と disabled controls に切り替わることを確認した。`POST /api/issues/00000000-0000-0000-0000-000000000000/publish` は 404 `issue not found` を返し route 疎通も確認。
- 2026-04-01 13:25:12 +0900 [codified-context] `codified-context-maintenance` を implementation-followup として開始。skill 本文と現 diff を確認し、inspection script / routing audit / commit まで実施する方針で着手。
- 2026-04-01 13:26:40 +0900 [codified-context] inspection script は mixed change set の manual triage を返したが、新規 durable asset は不要と判断。`AGENTS.md` と `context/repo-map.md` の repo summary に publish/read-only の current truth を追記した。
- 2026-04-01 14:10:13 +0900 [着手] 編集画面の「一覧へ戻る」横に古い案内/新しい案内ナビを追加する実装を開始。`/api/issues` の並び順を正本にし、detail API から前後情報を返す方針で着手。
- 2026-04-01 14:12:29 +0900 [実装] `src/main.rs` の issue detail response に `navigation.newer/older` を追加し、一覧と同じ `issue_month desc, created_at desc, id desc` 順で隣接 issue を返すようにした。`GET /api/issues` 側の order も同じ tie-break へ揃えた。
- 2026-04-01 14:14:13 +0900 [実装] `web-src/main.js` に `issueNavigation` state と nav normalize/render helper を追加し、編集画面の「一覧へ戻る」横へ `← 新しい案内` `古い案内 →` を描画するよう変更。`web-src/app.css` には nowrap と disabled 表示だけを最小追加した。
- 2026-04-01 14:15:31 +0900 [ビルド] `cargo fmt` と `npm run build` を実行。保存後 1 秒待機し、`build-error.txt` `test-error.txt` `clippy-error.txt` は成功、`build-error-win.txt` は未作成を確認した。
- 2026-04-01 14:15:31 +0900 [検証] `curl /api/issues/{id}` で `navigation.newer/older` を確認し、Chrome DevTools で最新号は `← 新しい案内` が disabled・`古い案内 →` が有効、中間号 `e773...` では両方向が有効で一覧順どおり遷移することを確認した。
- 2026-04-01 15:38:38 +0900 [着手] 編集画面の初回表示で preview が空白+「PDF を生成しています…」へ戻る不快感を解消するため、issue ごとの preview PNG をサーバー RAM 上へ 50MB LRU で保持し、ページロード直後に hydrate する修正へ着手。
- 2026-04-01 15:45:16 +0900 [実装] `src/main.rs` に `GET /api/issues/{id}/preview-cache` と `POST /api/preview-renders` 連動の 50MB issue preview PNG LRU を追加。key は `issue_id + source_version + font scale key` とし、editor/print の fresh load は RAM cache を返し、`manual-refresh` は bypass して再 rasterize する形へ変更した。
- 2026-04-01 15:45:16 +0900 [実装] `web-src/main.js` に `hydrateCachedIssuePreview()` と font scale key 生成を追加し、`loadEditor()` / `loadPrint()` は cached PNG があれば `renderEditor(skipPreview)` / `renderPrintPage(skipPreview)` で blank placeholder を出さずに初期表示するよう変更。dirty でない preview POST のみ server cache metadata を送るようにした。
- 2026-04-01 15:45:16 +0900 [保守] `context/repo-map.md` と `AGENTS.md` に常会案内 preview cache の current truth (`/api/issues/{id}/preview-cache`、50MB RAM LRU、font scale key 依存) を追記した。
- 2026-04-01 15:45:16 +0900 [検証] `cargo fmt` と `npm run build` 後、`build-error.txt` `test-error.txt` `clippy-error.txt` は成功、`build-error-win.txt` は未作成を確認。Chrome DevTools の network で2回目 reload は `/api/meta` `/api/issues/{id}` `/api/issues/{id}/preview-cache` のみで `/api/preview-renders` が走らず cached preview 表示になり、`再生成` ボタン押下時だけ `POST /api/preview-renders` が発火することを確認した。
- 2026-04-01 16:11:19 +0900 [着手] issue `f67f8fdc-4f4f-4605-8f85-65c78e652f66` の editor で無限チラつき調査を開始。`build-error.txt` `test-error.txt` `clippy-error.txt` は成功、`jokai-pdfme-layout` と `context/repo-map.md` を確認して preview 更新ループを追う方針にした。
- 2026-04-01 16:13:41 +0900 [調査] Chrome DevTools で対象 editor を観察し、`プレビュー生成に失敗しました: undefined` 表示中に `api/attachments/.../content#view=FitH` と PDF iframe の chrome-extension request が連打されることを確認。`web-src/main.js` の `generatePreview()` catch が `renderEditor()` / `renderPrintPage()` を再度呼び、`schedulePreview("render-editor")` を自己再起動しているのが無限チラつきの直接原因と判断。
- 2026-04-01 16:18:26 +0900 [実装] `web-src/main.js` に unknown error 整形 helper と `#global-flash-stack` 更新を追加し、常会案内の preview 失敗時は editor/print 全体を再描画せず flash と preview DOM だけ更新するよう変更。`renderEditor()` / `renderPrintPage()` 再入による自己ループを止めた。
- 2026-04-01 16:18:26 +0900 [ビルド] `npm run build` と `cargo fmt` を実行。保存後 1 秒待機し、`build-error.txt` `test-error.txt` `clippy-error.txt` は成功、`build-error-win.txt` は未作成を確認した。
- 2026-04-01 16:21:37 +0900 [調査] 再読込後の error は `The input is not a PNG file!`。`curl /api/attachments/.../thumbnail | xxd -l 16 -ps` を確認すると、`content-type:image/png` にもかかわらず先頭バイトが `%PDF-1.7` だった。`attachment_thumbnail_caches` または legacy thumbnail から PDF 本体を PNG と偽装して返しているのが preview 失敗の真因と判断。
- 2026-04-01 16:24:40 +0900 [実装] `src/main.rs` の attachment thumbnail 取得に PNG シグネチャ検証を追加。`pdf` 添付で cache/legacy thumbnail が PNG でない場合は破損 cache とみなして採用せず、原本 PDF から `pdftoppm` で PNG を再生成するよう修正した。legacy backfill 側も同じ偽 cache を新規投入しないよう防御した。
- 2026-04-01 16:24:40 +0900 [検証] `cargo fmt` 後の監視ファイルは成功。`curl /api/attachments/8f0a677f-f3a0-44a8-9e95-6f6bbdc2fd6f/thumbnail | xxd -l 16 -ps` は `89504e47...` を返し、Chrome DevTools で対象 editor を再読込すると無限チラつきは消え、A4 preview が表示され `更新済み` ステータスまで到達することを確認した。
- 2026-04-01 16:27:15 +0900 [codified-context] `codified-context-maintenance` を debug-followup として開始。inspection script と routing audit を実行し、新 asset は増やさず `AGENTS.md` / `context/repo-map.md` に preview flicker と偽 PNG thumbnail cache の current truth を追記する方針にした。
- 2026-04-01 16:29:04 +0900 [codified-context] `AGENTS.md` と `context/repo-map.md` に、`/api/attachments/{id}/thumbnail` の PNG シグネチャ検証 self-heal、notice preview 失敗時に全体再描画しない current truth、`The input is not a PNG file!` / 無限チラつき時の切り分け手順を追記した。
- 2026-04-01 16:53:01 +0900 [調査] issue `99b5758f-ddb6-4dcc-9212-474694125d9a` の editor preview を Chrome で再現確認。論理 2 ページの常会案内に対して、`next page` マーカーだけの白紙 2 ページ目が `pdfme` で挿入され、3 ページ目へ本文が送られる不具合と判断。
- 2026-04-01 16:59:33 +0900 [実装] `web-src/notice-pdf.js` で `次頁へ続く` を bottom-safe/no-halo へ変更し、bottom padding 超過 schema を reject する guard と論理 page count 返却を追加。`web-src/main.js` と `src/main.rs` には expected page count 検証、preview/list-thumbnail cache の render version 化、stale cache eviction を実装し、`db/011_issue_thumbnail_render_version.sql` と運用文書を追随させた。
- 2026-04-01 17:05:23 +0900 [検証] `npm run build` と `cargo fmt` 後、`build-error.txt` `test-error.txt` `clippy-error.txt` は成功、`build-error-win.txt` は未作成を確認。issue `99b5758f-...` の editor preview は 2 ページへ復帰し、P1 下端に `次頁へ続く`、P2 冒頭に `（続き 2頁）` を確認した。さらに hidden iframe で全 12 issue の `/print` preview を順次確認して 0 failure、一覧 `thumbnail_url` も全件 `image/png` / PNG signature 正常を確認した。
- 2026-04-01 17:11:20 +0900 [調査] ユーザー報告の「一つ前の変更でサムネイルの一部がおかしくなった」を受けて調査開始。`build-error.txt` と `test-error.txt` は成功、直近コミットは `5abf650 fix: 常会案内の継続マーカー白紙ページ化を防ぐ` で、render version と一覧サムネ経路の差分を優先確認する方針にした。
- 2026-04-01 17:18:55 +0900 [調査] ユーザー提示スクリーンショットの緑囲みは issue `12d76b7a-...` `7f89ad68-...` `99b5758f-...` の一覧サムネで再現。`curl /api/issues/{id}/list-thumbnail` と PNG 解析で 3 件とも `311x440 / 3596 bytes / SHA256=b1e95a...` の同一壊れ画像を返し、非白色ピクセルは `y=0..18` にしか存在しないことを確認した。
- 2026-04-01 17:18:55 +0900 [調査] 一方で `12d76b7a-.../print` と `7f89ad68-.../print` は正常紙面を表示し、`99b5758f-...` の editor preview も正常。元 issue データや `notice-pdf.js` 本体ではなく、`/api/issues/{id}/list-thumbnail` の生成タイミングまたはその cache 固定化が主因と判断した。
- 2026-04-01 17:26:48 +0900 [実装] `src/main.rs` の list thumbnail 再生成を、cache miss 時に headless Chrome `--dump-dom` で `/issues/{id}/print` を一度温めて preview-cache が DOM に載るまで待ってから `print-to-pdf` へ進む流れへ変更。既存の壊れた `issue_thumbnail_caches` と browser immutable cache をまとめて捨てるため `CURRENT_RENDERER_VERSION` を `pdfme-raster-v4` へ更新し、`AGENTS.md` と `context/repo-map.md` に current truth を追記した。
- 2026-04-01 17:26:48 +0900 [検証] `cargo fmt` 後に 1 秒待機し、`build-error.txt` / `test-error.txt` / `clippy-error.txt` は成功、`build-error-win.txt` は未作成を確認。`curl /api/issues` で `list-thumbnail?v=...pdfme-raster-v4` への切替を確認し、問題の `12d76b7a-...` `7f89ad68-...` `99b5758f-...` は新 URL で `24087/46662/40577 bytes` の正常 PNG に再生成、Chrome でも一覧サムネが復旧した。
- 2026-04-15 22:46:08 +0900 [着手] issue `b64db5f3-f9a6-446c-8aa7-2c98faed4384` の editor でタイトルが長いと改行される件の調査を開始。`build-error.txt` と `test-error.txt` は成功を確認し、`jokai-pdfme-layout`・`context/repo-map.md`・見本紙面を先に確認する方針にした。
- 2026-04-15 22:49:21 +0900 [調査] `web-src/notice-pdf.js` の `addFirstPageHeader()` と `addContinuationHeader()` がそれぞれ `width: 138/140mm` で `wrapText()` し、長い `issue.title` を 2 行化していることを確認。要求どおり単一行前提へ変更し、preview/list thumbnail cache を確実に捨てるため render version も更新する方針にした。
- 2026-04-15 22:50:31 +0900 [実装] `web-src/notice-pdf.js` の紙面タイトルを単一行前提へ変更し、header title box を可視ページ幅より広い固定幅にして `wrapText()` 依存を撤去。`src/main.rs` の `CURRENT_LAYOUT_VERSION` と `NOTICE_RENDER_VERSION` も `notice-pdf-layout-v4` へ更新して旧 preview/list thumbnail cache を無効化した。
- 2026-04-15 22:52:08 +0900 [検証] `npm run build` と `cargo fmt` 実施後、1秒待機して `build-error.txt` / `test-error.txt` / `clippy-error.txt` の成功と `build-error-win.txt` 未作成を確認。`http://localhost:12040/issues/b64db5f3-f9a6-446c-8aa7-2c98faed4384/edit` を reload して preview 再生成時刻更新を確認し、既存紙面が崩れていないことを Chrome で確認した。
- 2026-04-15 22:59:06 +0900 [着手] issue `b64db5f3-f9a6-446c-8aa7-2c98faed4384` の editor で `下へ` / `保存する` が `PUT /api/issues/{id}` 500 `db error` になる件の調査を開始。`build-error.txt` `test-error.txt` `clippy-error.txt` は成功を確認し、save payload と Rust 側更新処理の突き合わせから入る方針にした。
...
- 2026-06-16 13:38:23 +0900 [調査] Deck/Systemd/Caddy 対応作業を開始。`build-error.txt` / `test-error.txt` / `build-error-win.txt` / `clippy-error.txt` はいずれも未生成で、作業前の自動チェックエラーは確認できなかった。
- 2026-06-16 13:45:50 +0900 [実装] `JOKAI_BASE_PATH` と `X-Forwarded-Prefix` による subpath 配信対応を追加。HTML shell / template 印刷 HTML / issue 一覧サムネ URL / 添付 `thumbnail_url`・`content_url` / frontend fetch・href を base path 経由に寄せた。
- 2026-06-16 13:47:52 +0900 [実装] `scripts/install-user-systemd.sh` を追加し、`~/.config/systemd/user/jokai.service` と `~/.config/jokai/jokai.env` を生成して `jokai web` を user service として enable/start する導線を作成。`deck.toml` も build/install/db/systemd 操作用に拡張した。
- 2026-06-16 13:48:19 +0900 [設定] `/home/fusee/creat/fuse/caddy-project/Caddyfile` の `(toys)` に `/toys/jokai` slash redirect と `handle_path /toys/jokai/*` を追加し、`X-Forwarded-Prefix /toys/jokai` 付きで `127.0.0.1:12040` へ reverse proxy する設定にした。`root-index.html` にも jokai リンクを追加。
- 2026-06-16 13:50:58 +0900 [検証] `npm ci` で frontend 依存を復元後、`npm run build` が成功。`deck` は非対話の `deck deck.toml install` を受け付けなかったため TUI を PTY 操作し、`install` の `npm run build` と `cargo install --path . --force` は成功。TUI 復帰時のみ cursor position 取得エラーで deck process は exit 1 になった。
