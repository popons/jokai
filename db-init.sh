...
if [[ -n "${JOKAI_LEGACY_STORAGE_DIR}" ]]; then
  exec cargo run -- db init --database-url "${DATABASE_URL}" --admin-database-url "${DATABASE_ADMIN_URL}" --legacy-storage-dir "${JOKAI_LEGACY_STORAGE_DIR}"
else
  exec cargo run -- db init --database-url "${DATABASE_URL}" --admin-database-url "${DATABASE_ADMIN_URL}"
fi
