...
- `src/main.rs`: Axum エントリポイント。埋め込み frontend asset / 紙面用フォント配信、`issues` 系 CRUD、`template_documents` 系 CRUD、issue の draft 限定削除、発行 (`POST /api/issues/{id}/publish`)、一覧からの深い複製、添付の DB 保存、内部 temp dir を使う preview / thumbnail 生成、`/issues/{id}/print` と `/template-documents/{id}/print` の印刷 shell 配信を担当。`GET /api/issues` は一覧用 `thumbnail_url` を返し、`GET /api/issues/{id}/list-thumbnail` は `issue_thumbnail_caches` を使って issue 一覧向け 1 ページ目 PNG を返す。list thumbnail cache miss 時は、headless Chrome の `--dump-dom` で `/issues/{id}/print` を一度温めて preview-cache が DOM に載るまで待ってから PDF 化し、preview 未完成の白紙サムネ固定化を防ぐ。常会案内 editor / print 用 preview は `GET /api/issues/{id}/preview-cache` と `POST /api/preview-renders` の 50MB RAM LRU を持ち、`issue_id + source_version + render version + paper font scale key` 一致時は cached PNG を返す。`POST /api/preview-renders` は `expected_page_count` を受け取り、cached/fresh のどちらでも rasterized 枚数が一致しない preview を reject する。`/api/attachments/{id}/thumbnail` は `pdf` 添付の cache / legacy thumbnail を PNG シグネチャで検証し、偽 `image/png` cache を見つけたら原本 PDF から self-heal 再生成する。item/block 添付の追加・削除でも紙面見た目が変わる前提で `issues.source_version` を進める。`PUT /api/issues/{id}` の block / item 並べ替え保存では、`blocks(issue_id, sort_order)` / `block_items(block_id, sort_order)` の unique 衝突を避けるため、既存 row の `sort_order` を一旦退避値へ逃がしてから最終順を再採番する。`published` では save/delete だけでなく添付 mutation も拒否し、発行済み read-only の API 正本を守る。`農作業傷害共済` は raw SVG を返す `/template-documents/{id}/print` 正本 HTML、headless Chromium による `/api/template-documents/{id}/print-pdf`、`/api/template-documents/{id}/preview-images` もここで持つ。legacy filesystem 吸い上げは `db init` / `db migrate` の明示 `--legacy-storage-dir` 側へ分離した。`/api/issues/{id}/print-pdf` は互換メッセージのみで、server-side PDF生成は現状本線ではない。
- `web-src/main.js`: 文書ファミリー切替つき一覧画面、常会案内 editor、SVG テンプレ帳票 editor、状態管理、保存、発行、常時プレビュー再生成、一覧カードの `編集へ/閲覧へ` / `複製` / `削除` 導線、template document の rows[] 台帳 UI と補助 JSON editor、添付ビューア、item 添付欄での Clipboard 画像登録導線、item ごとの補足行群（赤/青）編集、`thumb_scale_percent` による項目サムネ倍率 slider を担当。常会案内一覧の `既存の案内` は `サムネ表示` / `詳細表示` を切り替えられ、選択状態は `localStorage` に保存する。常会案内 editor / print は起動時に server RAM cache の preview PNG を取りに行き、ヒット時は blank placeholder に戻さず cached 画像を先出しする。常会案内 preview 失敗時は editor / print 全体を再描画せず、global flash と preview DOM だけ更新して添付 iframe の再生成ループを避ける。常会案内 editor は draft では `発行する` を出し、published では read-only に切り替えて `複製して編集` 導線へ寄せる。editor 左カラムには sticky な表示コントロール帯があり、`号の骨格` / `文字サイズ` / `block-card` / `item-card` を個別または一括で折りたためる。折りたたみ時も title/要約は見えたままにし、折りたたみ状態は保存 payload に混ぜず UI state としてだけ扱う。小項目 toolbar は、heading/body/対象者/期限/補足行/添付のいずれかがあるときだけ `①` などの番号を表示する。`農作業傷害共済` editor は `payload.rows[]` を 1 行 1 ページ前提で編集し、行編集または valid JSON 変更後に auto-save して、その保存済み draft を使って `preview-images` を再取得する。
- `web-src/notice-pdf.js`: `pdfme` を使う A4 固定レイアウト生成の中核。本文左カラムと資料サムネ右カラムの設計、item ごとの赤/青補足行群の組版、画像実寸比を踏まえた右上基点の項目サムネ倍率反映、本文白まわりの可読性確保、halo の top padding 超過防止をここで守る。1ページ目タイトルと continuation title は単一行前提で自動改行させず、長い題名は運用側で短く調整する。小項目見出しの `①` などの番号は、紙面に出る内容が 1 つでもある item にだけ表示し、完全に空の item では番号も出さない。右脇の `【配布物】` / `【提出物】` などの赤ラベルも item ごとに 1 回だけ表示し、同一 item 内でサムネが複数あってもサムネ枚数分には増やさない。紙面内の見出し系・本文・補足・対象者/期限メタは赤い右ラベルの手前まで右へ伸ばし、サムネ重なりは許容するが、現在は太めにした白ハローで可読性を守る。最終ページ左下 footer メモも、右下連絡先ブロックの直前まで横に伸ばすが、footer 帯では halo を使わない。`次頁へ続く` も footer 帯と同じ bottom-safe text として扱い、bottom padding 内に収めて halo を載せない。page bottom 付近の halo text は `pdfme` が白紙ページを足す不具合を踏むため。
