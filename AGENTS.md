...
- 一覧画面の既存案内カード導線を触るときは、`web-src/main.js` の `renderIndex` と `src/main.rs` の `/api/issues`、`/api/issues/{id}`、`/api/issues/{id}/duplicate`、必要なら `/api/issues/{id}/list-thumbnail` をセットで確認すること。`published` の削除不可は UI と API の両方で守ること。
