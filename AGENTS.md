...
- `web-src/main.js`: 文書ファミリー切替つき一覧画面、常会案内 editor、SVG テンプレ帳票 editor、状態管理、保存、発行、常時プレビュー再生成、一覧カードの `編集へ/閲覧へ` / `複製` / `削除` 導線、template document の rows[] 台帳 UI と補助 JSON editor、添付ビューア、item 添付欄での Clipboard 画像登録導線、item ごとの補足行群（赤/青）編集、`thumb_scale_percent` による項目サムネ倍率 slider を担当。常会案内一覧の `既存の案内` は `サムネ表示` / `詳細表示` を切り替えられ、選択状態は `localStorage` に保存する。常会案内 editor / print は起動時に server RAM cache の preview PNG を取りに行き、ヒット時は blank placeholder に戻さず cached 画像を先出しする。常会案内 preview 失敗時は editor / print 全体を再描画せず、global flash と preview DOM だけ更新して添付 iframe の再生成ループを避ける。常会案内 editor は draft では `発行する` を出し、published では read-only に切り替えて `複製して編集` 導線へ寄せる。editor 左カラムには sticky な表示コントロール帯があり、`号の骨格` / `文字サイズ` / `block-card` / `item-card` を個別または一括で折りたためる。初期表示は `全部折りたたみ` 相当で、fold state は保存 payload に混ぜず UI state としてだけ扱う。各 block-card には `小項目番号:表示/非表示` の UI-only トグルがあり、issue ごとの localStorage を通して editor / print / PDF へ同じ設定を渡す。小項目 toolbar の `①` などの番号は、その block が `表示` のときだけ heading/body/対象者/期限/補足行/添付のいずれかがある item に出す。`農作業傷害共済` editor は `payload.rows[]` を 1 行 1 ページ前提で編集し、行編集または valid JSON 変更後に auto-save して、その保存済み draft を使って `preview-images` を再取得する。
...
- `web-src/app.css`: 編集UIとプレビューUIの見た目。項目サムネ倍率 slider のような非印刷UIの密度もここで保つ。常会案内一覧の `サムネ表示` / `詳細表示` toggle と A4 サムネ gallery の見た目もここで管理する。editor の sticky 表示コントロール帯、折りたたみ header、要約表示、responsive 時の並び替えもここで管理する。紙面そのもののレイアウト原則は `notice-pdf.js` 側が主。
