...
- `src/main.rs`: Axum エントリポイント。埋め込み frontend asset / 紙面用フォント配信、`issues` 系 CRUD、`template_documents` 系 CRUD、issue の draft 限定削除、発行 (`POST /api/issues/{id}/publish`)、一覧からの深い複製、添付の DB 保存、内部 temp dir を使う preview / thumbnail 生成、`/issues/{id}/print` と `/template-documents/{id}/print` の印刷 shell 配信を担当。`JOKAI_BASE_PATH` と `X-Forwarded-Prefix` による subpath 配信にも対応し、HTML shell / API JSON 内 URL は base path 付きで返す。root 直アクセスと `JOKAI_BASE_PATH` 配下の直アクセスの両方を受けるため、同一 routes を root と base path に載せている。web 起動ログでは実際の listener port と base path からコピー可能な loopback URL を出し、all-interface bind 時は取得できた primary local IP URL と bound address を分けて出す。`GET /api/issues` は一覧用 `thumbnail_url` を返し、`GET /api/issues/{id}/list-thumbnail` は `issue_thumbnail_caches` を使って issue 一覧向け 1 ページ目 PNG を返す。list thumbnail cache miss 時は、headless Chrome の `--dump-dom` で `/issues/{id}/print` を一度温めて preview-cache が DOM に載るまで待ってから PDF 化し、preview 未完成の白紙サムネ固定化を防ぐ。常会案内 editor / print 用 preview は `GET /api/issues/{id}/preview-cache` と `POST /api/preview-renders` の 50MB RAM LRU を持ち、`issue_id + source_version + render version + paper display key` 一致時は cached PNG を返す。`paper display key` は browser-local な文字倍率と issue ごとの block 単位小項目番号設定を含む。`POST /api/preview-renders` は `expected_page_count` を受け取り、cached/fresh のどちらでも rasterized 枚数が一致しない preview を reject する。`/api/attachments/{id}/thumbnail` は `pdf` 添付の cache / legacy thumbnail を PNG シグネチャで検証し、偽 `image/png` cache を見つけたら原本 PDF から self-heal 再生成する。item/block 添付の追加・削除でも紙面見た目が変わる前提で `issues.source_version` を進める。`PUT /api/issues/{id}` の block / item 並べ替え保存では、`blocks(issue_id, sort_order)` / `block_items(block_id, sort_order)` の unique 衝突を避けるため、既存 row の `sort_order` を一旦退避値へ逃がしてから最終順を再採番する。`published` では save/delete だけでなく添付 mutation も拒否し、発行済み read-only の API 正本を守る。`農作業傷害共済` は raw SVG を返す `/template-documents/{id}/print` 正本 HTML、headless Chromium による `/api/template-documents/{id}/print-pdf`、`/api/template-documents/{id}/preview-images` もここで持つ。legacy filesystem 吸い上げは `db init` / `db migrate` の明示 `--legacy-storage-dir` 側へ分離した。`/api/issues/{id}/print-pdf` は互換メッセージのみで、server-side PDF生成は現状本線ではない。
- `web-src/main.js`: 文書ファミリー切替つき一覧画面、常会案内 editor、SVG テンプレ帳票 editor、状態管理、保存、発行、常時プレビュー再生成、一覧カードの `編集へ/閲覧へ` / `複製` / `削除` 導線、template document の rows[] 台帳 UI と補助 JSON editor、添付ビューア、item 添付欄での Clipboard 画像登録導線、item ごとの補足行群（赤/青）編集、`thumb_scale_percent` による項目サムネ倍率 slider を担当。HTML shell の `data-base-path` を使い、fetch / href / attachment viewer URL を subpath-aware にする。常会案内一覧の `既存の案内` は `サムネ表示` / `詳細表示` を切り替えられ、選択状態は `localStorage` に保存する。常会案内 editor / print は起動時に server RAM cache の preview PNG を取りに行き、ヒット時は blank placeholder に戻さず cached 画像を先出しする。常会案内 preview 失敗時は editor / print 全体を再描画せず、global flash と preview DOM だけ更新して添付 iframe の再生成ループを避ける。常会案内 editor は draft では `発行する` を出し、published では read-only に切り替えて `複製して編集` 導線へ寄せる。editor 左カラムには sticky な表示コントロール帯があり、`号の骨格` / `文字サイズ` / `block-card` / `item-card` を個別に折りたため、`全展開` / `中間` / `全閉じ` preset で一括切替できる。初期表示は `中間` 相当で、`号の骨格` / `文字サイズ` と item-card は畳み、block-card だけ開く。fold state は保存 payload に混ぜず UI state としてだけ扱う。item 本文欄の小型 `B` ボタンは選択範囲を raw text の `**...**` で囲むだけで、保存 payload は本文文字列のまま扱う。各 block-card には `小項目番号:表示/非表示` の UI-only トグルがあり、issue ごとの localStorage を通して editor / print / PDF へ同じ設定を渡す。小項目 toolbar の `①` などの番号は、その block が `表示` のときだけ heading/body/対象者/期限/補足行/添付のいずれかがある item に出す。`農作業傷害共済` editor は `payload.rows[]` を 1 行 1 ページ前提で編集し、行編集または valid JSON 変更後に auto-save して、その保存済み draft を使って `preview-images` を再取得する。
- `web-src/notice-pdf.js`: `pdfme` を使う A4 固定レイアウト生成の中核。本文左カラムと資料サムネ右カラムの設計、item ごとの赤/青補足行群の組版、画像実寸比を踏まえた右上基点の項目サムネ倍率反映、本文白まわりの可読性確保、halo の top padding 超過防止をここで守る。item 本文 `body` 内の同一行 `**...**` だけを赤太字 inline 強調に変換し、`**` marker は紙面に出さない。未閉じ `**` は通常文字として扱い、見出し・補足行・対象者/期限メタ・footer には適用しない。font asset と添付サムネ取得も `data-base-path` 由来の URL helper を通す。1ページ目タイトルと continuation title は単一行前提で自動改行させず、長い題名は運用側で短く調整する。小項目見出しの `①` などの番号は issue ごとの block 単位 `show_item_markers` map が ON かつ紙面に出る内容が 1 つでもある item にだけ表示し、OFF のときは heading だけを出し、heading も空なら見出し行自体を潰す。直前 item が見出しのみで現在 item が同一ページに続く場合、現在 item のサムネと右ラベルだけを直前見出し行の右へ上げ、本文位置や item ownership は動かさない。右脇の `【配布物】` / `【提出物】` などの赤ラベルも item ごとに 1 回だけ表示し、同一 item 内でサムネが複数あってもサムネ枚数分には増やさない。紙面内の見出し系・本文・補足・対象者/期限メタは赤い右ラベルの手前まで右へ伸ばし、サムネ重なりは許容するが、現在は太めにした白ハローで可読性を守る。最終ページ左下 footer メモも、右下連絡先ブロックの直前まで横に伸ばすが、footer 帯では halo を使わない。`次頁へ続く` も footer 帯と同じ bottom-safe text として扱い、bottom padding 内に収めて halo を載せない。page bottom 付近の halo text は `pdfme` が白紙ページを足す不具合を踏むため。
- `web-src/template-doc-registry.js`: `農作業傷害共済` 用テンプレ定義、binding キー、`payload.rows[]` の default/normalize、行単位必須チェック、タイトル自動生成規則の正本。
- `web-src/template-doc-pdf.js`: `templateDocumentPdfFileName()` と rows[] 対応の browser-side 補助 builder を持つ補助モジュール。font asset は `data-base-path` 由来の URL helper で取得する。`農作業傷害共済` の本線 preview / print / PDF 出力は Chromium 経路が正本で、ここは補助扱い。
- `web-src/app.css`: 編集UIとプレビューUIの見た目。項目サムネ倍率 slider のような非印刷UIの密度もここで保つ。常会案内一覧の `サムネ表示` / `詳細表示` toggle と A4 サムネ gallery の見た目もここで管理する。editor の sticky 表示コントロール帯、折りたたみ header、要約表示、展開中 item-card の枠強調、responsive 時の並び替えもここで管理する。紙面そのもののレイアウト原則は `notice-pdf.js` 側が主。
- `db/*.sql`: PostgreSQL マイグレーション。`issues` / `blocks` / `block_items` / `block_item_supplements` / `attachments` / `template_documents` に加え、`block_items.thumb_scale_percent`、添付原本の `attachment_original_contents`、サムネ cache の `attachment_thumbnail_caches` と `issue_thumbnail_caches`、legacy な `generated_files` を定義する。`010_reset_issue_thumbnail_caches.sql` は既存誤 cache の一括再生成用、`011_issue_thumbnail_render_version.sql` は issue 一覧サムネ cache に render version を持たせて layout/renderer 更新時の stale reuse を止める migration。
...
- `deck.toml`: `watch build` / `run server` / `npm run build` / `cargo install --path . --force` / DB 初期化 / user systemd 操作をまとめるローカル運用入口。
- `scripts/install-user-systemd.sh`: `~/.config/systemd/user/jokai.service` と `~/.config/jokai/jokai.env` を生成し、`jokai web` を user service として enable/start する installer。
...
- 一覧画面や発行導線を触るときは、`web-src/main.js` の `renderIndex` / `renderEditor` と `src/main.rs` の `/api/issues`、`/api/issues/{id}`、`/api/issues/{id}/publish`、`/api/issues/{id}/duplicate`、必要なら `/api/issues/{id}/list-thumbnail` をセットで確認すること。`published` の削除不可と read-only は UI と API の両方で守ること。
- 常会案内の初回 preview 表示や blank placeholder を触るときは、`web-src/main.js` の `hydrateCachedIssuePreview()` / `rasterizePreview()`、`src/main.rs` の `/api/issues/{id}/preview-cache` / `/api/preview-renders`、`issues.source_version` をセットで確認すること。`paper display key` または render version が変わると cache miss になる。`paper display key` には文字倍率と issue ごとの block 単位小項目番号設定が含まれる。`expected_page_count` 不一致で preview が reject されたら、白紙ページ混入や cache 汚染を疑うこと。
- 常会案内 editor が無限にチラつく、または `The input is not a PNG file!` で preview 失敗するときは、`web-src/main.js` の `generatePreview()` catch と `src/main.rs` の `/api/attachments/{id}/thumbnail` self-heal を確認し、必要なら `curl /api/attachments/{id}/thumbnail | xxd -l 16 -ps` で実バイト先頭が `89504e47` か `%PDF` かを確認すること。偽 `image/png` cache は preview ループの温床になる。
- 一覧サムネ不一致を触るときは、`src/main.rs` の `issue_thumbnail_caches` / `issues.source_version` / 添付追加削除時の version bump、`web-src/main.js` の `thumbnail_url?v=...` と browser cache をセットで確認すること。
...
- 折りたたみ挙動を触るときは、`web-src/main.js` の `renderEditor` / `renderItemCard` / `bindEditEvents` と `web-src/app.css` の sticky toolbar / summary layout をセットで確認すること。折りたたみ状態を issue/block/item の保存 payload に混ぜないこと。
...
- subpath 配信や Caddy 公開を触るときは、`src/main.rs` の `JOKAI_BASE_PATH` / `X-Forwarded-Prefix` 解決、`web-src/main.js` / `web-src/notice-pdf.js` / `web-src/template-doc-pdf.js` の URL helper、`deck.toml` / `scripts/install-user-systemd.sh` の既定 env、外部 `/home/fusee/creat/fuse/caddy-project/Caddyfile` の `/toys/jokai` proxy をセットで確認すること。Caddy 側は `handle_path /toys/jokai/*` で upstream root へ渡し、`X-Forwarded-Prefix /toys/jokai` を付ける。
...
- item 本文の `**...**` 強調を触るときは、`web-src/main.js` の本文欄 `B` ボタンと raw text 保存、`web-src/notice-pdf.js` の本文専用 inline parser / 分割 text schema / page height 計算をセットで確認すること。Markdown 全般へ広げず、補足行・メタ・footer へ混ぜないこと。
- 小項目の `①` などの表示を触るときは、`web-src/main.js` の block-card 単位トグルと item toolbar、`web-src/notice-pdf.js` の紙面見出しを両方確認し、block ごとの ON/OFF が editor / preview / PDF で揃うこと、空の item では番号を出さないことを確認すること。
- 本文右余白やサムネ重なりを触るときは、`web-src/notice-pdf.js` の本文系 width 定数と `pushTextSchema()` の halo をセットで確認し、赤い右ラベルを侵食せずに本文だけを右へ伸ばすこと。重なり可読性は強めの white halo に依存しているので、halo を戻したり細くする変更は同時に実画面確認まで行うこと。
- footer の横幅を触るときは、左下赤メモを右下連絡先ブロックの直前まで広げる前提を崩さず、連絡先ブロック自体は固定のまま守ること。footer 帯で halo を復活させないこと。page bottom 付近の halo text は `pdfme` が白紙 2 ページ目を足す。
...
- block / item の並べ替え保存不具合を触るときは、`db/001_init.sql` / `db/002_block_items.sql` の `unique(..., sort_order)` と `src/main.rs` の一時退避更新をセットで確認すること。単純な連番 update へ戻すと再び `db error` になる。
...
- `cargo fmt`: 変更後に実行が必要。
- `deck install`: `npm run build` 後に `cargo install --path . --force` を実行し、`~/.cargo/bin/jokai` を更新する。現行 `deck` は非対話で特定 command を直接実行する CLI ではなく、TUI から選択実行する。
- `deck systemd install`: `scripts/install-user-systemd.sh` を実行し、既定では `DATABASE_URL=postgresql://postgres:postgres@10.0.0.100:5432/jokai`、`JOKAI_BIND=0.0.0.0:12040`、`JOKAI_BASE_PATH=/toys/jokai` で `jokai.service` を enable/start する。
- `deck systemd status` / `deck systemd logs`: user service の状態確認と journald tail。手動なら `systemctl --user status jokai.service --no-pager` / `journalctl --user -u jokai.service -f`。
