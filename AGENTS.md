...
- 常会案内の初回 preview 表示や blank placeholder を触るときは、`web-src/main.js` の `hydrateCachedIssuePreview()` / `rasterizePreview()`、`src/main.rs` の `/api/issues/{id}/preview-cache` / `/api/preview-renders`、`issues.source_version` をセットで確認すること。`font scale key` が変わると cache miss になる。
