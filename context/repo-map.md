...
| Path | Role |
|---|---|
| `src/main.rs` | Axum サーバー、埋め込み HTML/JS/CSS/紙面フォント配信、`issues` 系 CRUD、`template_documents` 系 CRUD、draft 限定削除、発行 (`POST /api/issues/{id}/publish`)、issue 深い複製、添付の DB 保存、内部 temp dir を使う `pdftoppm` preview / thumbnail 生成、印刷用 shell 配信を担当。`JOKAI_BASE_PATH` と `X-Forwarded-Prefix` による subpath 配信に対応し、HTML shell、template 印刷 HTML、API JSON 内の `thumbnail_url` / `content_url` を base path 付きで返す。root 直アクセスと `JOKAI_BASE_PATH` 配下の直アクセスの両方を受けるため、同一 routes を root と base path に載せる。web 起動ログでは listener 実ポートと base path からコピー可能な loopback URL を出し、all-interface bind 時は取得できた primary local IP URL と bound address を分けて出す。`issues.agenda_label` もここで read/save/duplicate し、常会案内を総会案内へ転用するときの議題見出し正本を保持する。`GET /api/issues` では一覧用 `thumbnail_url` も返し、`GET /api/issues/{id}/list-thumbnail` は print shell 由来 PDF の 1ページ目 PNG を cache 付きで返す。list thumbnail cache miss 時は、headless Chrome の `--dump-dom` で `/issues/{id}/print` を一度温めて preview-cache が DOM に載るまで待ってから PDF 化し、preview 未完成の白紙サムネ固定化を防ぐ。常会案内 editor / print 用の preview PNG は `GET /api/issues/{id}/preview-cache` と `POST /api/preview-renders` の 50MB RAM LRU を持ち、`issue_id + source_version + render version + paper display key` 一致時は初回表示で cached preview を返す。`paper display key` は browser-local な文字倍率と issue ごとの block 単位小項目番号設定を含む。`POST /api/preview-renders` は `expected_page_count` を受け取り、cached/fresh のどちらでも rasterized 枚数が一致しない preview を reject する。`/api/attachments/{id}/thumbnail` は `pdf` 添付の cache / legacy thumbnail を PNG シグネチャで検証し、偽 `image/png` cache を見つけたら原本 PDF から self-heal 再生成する。`PUT /api/issues/{id}` の block / item 並べ替え保存では、`blocks(issue_id, sort_order)` / `block_items(block_id, sort_order)` の unique 制約と衝突しないよう、既存 row の `sort_order` を一旦退避値へ逃がしてから最終順へ再採番する。`published` では save/delete だけでなく添付 mutation も拒否し、API 側の read-only を保証する。`農作業傷害共済` は `payload.rows[]` を single object 互換で正規化し、行数ベースの自動タイトルを返し、server-rendered `/template-documents/{id}/print` 正本 HTML、headless Chromium による `print-pdf` 生成、`preview-images` API もここで持つ。legacy filesystem 吸い上げは web 起動時ではなく DB コマンドの明示 `--legacy-storage-dir` 側に寄せた |
| `web-src/main.js` | 文書ファミリー切替付き一覧画面、常会案内 editor、SVG テンプレ帳票 editor の状態管理、CRUD 呼び出し、発行、一覧カード操作、常時プレビュー、rows[] 台帳 UI、補助 JSON editor、item 添付欄での Clipboard 画像登録、item ごとの補足行群（赤/青）編集、`meta_layout` による対象者/期限の並び替え、`thumb_scale_percent` による項目単位サムネ倍率 slider を担当。HTML shell の `data-base-path` を使い、fetch / href / attachment viewer URL を subpath-aware にする。常会案内一覧の `既存の案内` は `サムネ表示` / `詳細表示` を切り替えられ、選択状態は `localStorage` に保存する。常会案内 editor / print は起動時に `render version + paper display key` 付きで cached preview を取りに行き、ヒット時は blank placeholder へ戻さず cached PNG を先に表示する。常会案内 preview 生成時は `buildNoticePdfDocument()` が返す論理 page count を `/api/preview-renders` へ送り、server rasterized 枚数が合わない preview を reject させる。常会案内 preview 失敗時は editor / print 全体を再描画せず、global flash と preview DOM だけ更新して添付 iframe の再生成ループを避ける。draft では `発行する` を出し、published では read-only 表示と `複製して編集` 導線に切り替える。editor 左カラムには sticky な表示コントロール帯があり、`号の骨格` / `文字サイズ` / `block-card` / `item-card` を個別に折りたため、`全展開` / `中間` / `全閉じ` preset で一括切替できる。初期表示は `中間` 相当で、`号の骨格` / `文字サイズ` と item-card は畳み、block-card だけ開く。fold state は保存 payload に含めない UI-only state とする。item 本文欄の小型 `B` ボタンは選択範囲を raw text の `**...**` で囲むだけで、保存 payload は本文文字列のまま扱う。各 block-card には `小項目番号:表示/非表示` の UI-only トグルがあり、issue ごとの localStorage を通して editor / preview / PDF へ同じ設定を渡す。小項目 toolbar の `①` などの番号は、その block が `表示` のときだけ heading/body/対象者/期限/補足行/添付のいずれかがある item に表示する。`農作業傷害共済` editor では 1 行 1 ページ前提で `payload.rows[]` を編集し、行編集または valid JSON 入力時に auto-save 後、server-side `preview-images` を再取得する |
| `web-src/notice-pdf.js` | `pdfme` で A4 固定レイアウトを組み立てる紙面生成本体。item ごとの赤/青補足行群を本文直下へ入力順で縦積みし、`meta_layout` に応じて対象者/期限を同一行または縦積みで組版する。本文 `item.body`、補足行、対象者/期限メタ行は紙面上で一文字インデントする。本文 `item.body` 内の同一行 `**...**` だけを赤太字 inline 強調として扱い、`pdfme` の制約上は複数 text schema に分割して描画する。未閉じ `**` は通常文字として残し、見出し・補足行・対象者/期限メタ・footer には適用しない。font asset と添付サムネ fetch は `data-base-path` 由来の URL helper を通す。1ページ目の大見出しと `agenda` block の既定ラベルは `issue.agenda_label` を使い、未設定時だけ `常会事項` にフォールバックする。1ページ目タイトルと continuation title は単一行前提で自動改行させず、長いタイトルは利用者側で短く調整する。小項目見出しの `①` などの番号は issue ごとの block 単位 `show_item_markers` map が ON かつ紙面に出る内容が 1 つでもある item にだけ表示し、OFF のときは heading だけを出し、heading も空なら見出し行自体を潰す。直前 item が見出しのみで現在 item が同一ページに続く場合、現在 item のサムネと右ラベルだけを直前見出し行の右へ上げる。本文位置と item / attachment の ownership は動かさない。右脇の `【配布物】` / `【提出物】` などの赤ラベルも item ごとに 1 回だけ表示し、同一 item 内でサムネが複数あってもラベルは重複させない。紙面内の見出し系・本文・補足・対象者/期限メタは赤い右ラベルの手前まで右へ伸ばし、サムネ重なりを許容する。最終ページ左下 footer メモも右下連絡先ブロックの直前まで横に伸ばす。右脇サムネは `thumb_scale_percent` で項目ごとに 80〜200% を右上基点で拡縮し、`measureImageDataUrl()` で実画像アスペクト比を測って「見えている画像」自体の right-top を固定する。画像自体は全テキストの背面へ置き、`pushTextSchema()` の halo は blank page の top padding を割らないよう clamp しつつ、現在は倍化した強めの white halo で重なった本文の可読性を守る。ただし footer 帯と `次頁へ続く` は例外で、bottom-safe text として bottom padding 内に収め、halo を載せない。page bottom 付近の halo text は `pdfme` の白紙ページ追加 bug を踏む。`validateTemplate()` は top/bottom padding を越える schema を reject する。`loadFonts()` は static instance の body/body-bold/title を読み込み、Thin 側へ落ちないようにしている |
...
| `web-src/template-doc-pdf.js` | `templateDocumentPdfFileName()` と rows[] 対応の browser-side 補助 builder を持つ補助モジュール。font asset は `data-base-path` 由来の URL helper で取得する。`農作業傷害共済` の本線 preview / print / PDF 出力は 2026-03-19 時点で Chromium 経路が正本 |
| `web-src/app.css` | 編集画面/プレビュー画面の UI スタイル。非印刷UIの余白密度、sticky な表示コントロール帯、折りたたみ header / summary、展開中 item-card の枠強調、responsive 時の並び替えもここで制御する |
...
| `scripts/install-user-systemd.sh` | `~/.config/systemd/user/jokai.service` と `~/.config/jokai/jokai.env` を生成し、`jokai web` を user service として enable/start する installer。既定 base path は `/toys/jokai` |
...
同じ route は root 直下と `JOKAI_BASE_PATH` 配下の両方で受ける。公開運用では外部 `/home/fusee/creat/fuse/caddy-project/Caddyfile` の `(toys)` snippet が `handle_path /toys/jokai/*` で upstream `127.0.0.1:12040` へ渡し、`X-Forwarded-Prefix /toys/jokai` を付ける。server 側は header を優先し、なければ `JOKAI_BASE_PATH` を使って HTML asset URL と API JSON 内 URL を組み立てる。
...
-> `PUT /api/issues/{id}` で保存し、`issues.source_version` を加算する。block / item 並べ替えを含む場合は、server 側で既存 `sort_order` を一旦退避値へ逃がしてから最終順へ再採番し、unique 制約衝突による `db error` を避ける
...
`web-src/notice-pdf.js` の `buildNoticePdfDocument(issue, blocks, fontScale, renderOptions)`  
-> ブラウザ内で notice PDF bytes を生成  
-> fresh page load では先に `GET /api/issues/{id}/preview-cache?font_scale_key=...` を叩き、`issue_id + source_version + render version + paper display key` 一致の cached PNG があればそれを即表示する  
...
-> `web-src/main.js` が issue / blocks を読み込み、編集画面と同じ `buildNoticePdfDocument(issue, blocks, fontScale, renderOptions)` で PDF bytes を生成  
...
| item 本文・補足行・メタの表示 | 本文 `item.body`、補足行、対象者/期限メタ行は紙面上で一文字インデントする。本文内の同一行 `**...**` だけは赤太字 inline 強調になり、marker 自体は紙面に出ない。補足行は `block_item_supplements` の入力順で本文直下に縦積みし、`block_items.meta_layout` は対象者/期限だけを `same_line` / `stacked` で制御する |
| 項目サムネ倍率 | `block_items.thumb_scale_percent` は 80〜200 の 5 刻み。right-top は schema box ではなく見えている画像自体の右上固定を意味し、本文位置は動かさない。直前 item が見出しのみで同一ページに続くときは、次 item のサムネと右ラベルだけを見出し行の右に上げる。縦方向の重なりは許容し、画像は常に全テキストの背面へ置く |
...
- `deck install` は TUI から選択実行し、`npm run build` と `cargo install --path . --force` を順に走らせる。現行 `deck` は `deck deck.toml install` のような非対話実行を受け付けない。
- `deck systemd install` は `scripts/install-user-systemd.sh` を実行し、`jokai.service` を user service として enable/start する。既定 env は `DATABASE_URL=postgresql://postgres:postgres@10.0.0.100:5432/jokai`、`JOKAI_BIND=0.0.0.0:12040`、`JOKAI_BASE_PATH=/toys/jokai`。
- service 検証は `systemctl --user status jokai.service --no-pager`、`curl http://127.0.0.1:12040/healthz`、`curl http://127.0.0.1:12040/toys/jokai/healthz`、`curl -H 'X-Forwarded-Prefix: /toys/jokai' http://127.0.0.1:12040/` を使う。
- Caddy の syntax だけ見るなら `caddy adapt --config /home/fusee/creat/fuse/caddy-project/Caddyfile`。`caddy validate` は実証明書 `/etc/letsencrypt/live/toy.fuse.co.jp/fullchain.pem` の読取権限が必要で、権限不足だと config 内容に関係なく失敗する。
...
- 小項目番号表示を触ったら、block ごとの ON/OFF が editor / preview / PDF で揃うこと、空の item では editor toolbar も紙面見出しも `①` を出さず、heading/body/対象者/期限/補足行/添付のどれかが入った時点で番号が出ることを確認する。
